...
class LangChainChatWrapperBase:
    ...
    def _build_messages(
        self, system_message: Optional[str], human_message: str
    ) -> List:
        ...
        try:
            ...
            if temperature is not None or top_p is not None or kwargs or system_message:
                ...
                if system_message:
                    ...
                    chain = (
                        RunnableLambda(format_messages) | temp_llm | self.output_parser
                    )
                    result = chain.invoke(
                        {"system_prompt": system_message, "message": prompt}
                    )
                else:
    ...
    def _generate_with_tools(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.info(
                f"Tool generation completed. Final response length: {len(final_response) if isinstance(final_response, str) else 'N/A'}"
            )
        ...
        **kwargs,
        ...
        try:
            ...
            llm = self._create_llm(
                temperature=effective_temperature, top_p=top_p, **kwargs
            )
            llm_with_tools = llm.bind_tools(tools) if tools else llm
            ...
            json_instruction = self._build_schema_instruction(
