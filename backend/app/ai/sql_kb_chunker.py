...
class SQLKnowledgeBaseChunker:
        ...
        try:
            ...
            if not db_info:
                self.parsing_errors.append(
                    f"Failed to parse database info from {file_path}"
                )
                return []
            ...
            for table in tables:
                ...
                table_summary_chunk = self._create_table_summary_chunk(
                    table,
                    db_info["db_name"],
                    db_info.get("module_name", "unknown"),
                    db_info.get("purpose", ""),
                )
    ...
    def _parse_columns(self, columns_text: str) -> List[Dict[str, str]]:
        ...
        for match in re.finditer(column_pattern, columns_text):
            ...
            if (
                col_name in ("Column", "---")
                or col_name.strip().replace("-", "").replace(" ", "") == ""
            ):
                continue
    ...
    def _create_database_chunk(
        self, db_info: Dict[str, Any], tables: List[Dict[str, Any]]
    ) -> DatabaseChunk:
    ...
    def _create_table_summary_chunk(
        ...
        return TableChunk(
            chunk_id=chunk_id, chunk_type="table", content=content, metadata=metadata
        )

    def _create_table_columns_chunk(
        self, table: Dict[str, Any], db_name: str, module_name: str
    ) -> ColumnChunk:
        ...
        content_lines = [f"Table: {table_name}", f"Purpose: {table_purpose}", ""]
        ...
        return ColumnChunk(
            chunk_id=chunk_id, chunk_type="column", content=content, metadata=metadata
        )
    ...
    def save_chunks_to_json(
        self,
        chunks: List[Union["DatabaseChunk", "TableChunk", "ColumnChunk"]],
        output_path: str,
    ):
...
def main():
    ...
    parser.add_argument(
        "output_dir", help="Folder where output JSON files will be saved"
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
    ...
    if os.path.isfile(input_path):
        ...
        print(
            f"Database chunks: {len([c for c in chunks if c.chunk_type == 'database'])}"
        )
