...
class ChunkParsers:
    ...
    def format_database_chunks(
        ...
        for i, db_info in enumerate(db_infos, 1):
            ...
            formatted_sections.append(
                f"--- **Database {i}: {db_info.get('database', 'Unknown')}** ---"
            )
            ...
            formatted_sections.append(
                f"**Purpose:** {db_info.get('purpose', 'Not specified')}"
            )
            ...
            if tables:
                ...
                for table in tables[:5]:
                    ...
                    if ":" in table:
                        ...
                        formatted_sections.append(
                            f"  • {table_name.strip()}: {description.strip()}"
                        )
                    else:
    ...
    def format_table_chunks(
        ...
        for i, table_info in enumerate(table_infos, 1):
            formatted_sections.append(
                f"--- **Table {i}: {table_info.get('table', 'Unknown')}** ---"
            )
            formatted_sections.append(
                f"**Database:** {table_info.get('database', 'Unknown')}"
            )
            formatted_sections.append(
                f"**Purpose:** {table_info.get('purpose', 'Not specified')}"
            )
    ...
    @staticmethod
    def format_table_chunks_filtered_by_databases(
        results: Dict[str, Any], selected_databases: List[str]
    ) -> str:
        ...
        for table_name, columns in table_columns.items():
            ...
            if columns:
                ...
                for col in sorted_columns:
                    formatted_sections.append(
                        f"  {ChunkParsers._format_single_column(col)}"
                    )
            else:
                formatted_sections.append(
            ...
            for line in lines:
                ...
                if len(remaining_parts) > 0:
                    ...
                    if first_part in ["pri", "uni", "mul"]:
                        ...
                        nullable = (
                            remaining_parts[1] if len(remaining_parts) > 1 else "yes"
                        )
                        extra_start_idx = 2
                ...
                extra_parts = (
                    remaining_parts[extra_start_idx:]
                    if len(remaining_parts) > extra_start_idx
                    else []
                )
                ...
                if extra_parts:
                    if is_auto_increment:
                        ...
                        if (
                            first_part
                            in ["null", "0", "1", "current_timestamp", "''", '""']
                            or first_part.isdigit()
                        ):
    ...
    @staticmethod
    def parse_table_chunk_with_keys(
        chunk_content: str, metadata: dict
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def parse_database_chunk_with_tables(
        chunk_content: str, metadata: dict
    ) -> Dict[str, Any]:
