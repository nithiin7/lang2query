    ...
    try:
        ...
        if (response_str.startswith('{') and response_str.endswith('}')) or \
           (response_str.startswith('[') and response_str.endswith(']')):
            ...
            _log_text_response(logger, response_str)
    except (json.JSONDecodeError, TypeError):
        # Not valid JSON, handle as regular text
        _log_text_response(logger, str(response))
...
def log_query(logger, query: str, confidence: float = None):
    ...
    if confidence is not None:
        color = Colors.BRIGHT_GREEN if confidence > 0.7 else Colors.BRIGHT_YELLOW if confidence > 0.4 else Colors.BRIGHT_RED
        logger.info(f"   {Colors.BOLD}Confidence: {color}{confidence:.2f}{Colors.RESET}")
...
def _detect_query_type(query: str) -> str:
    ...
    if any(keyword in query_upper for keyword in ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'CREATE', 'DROP']):
        return 'sql'
    elif query.strip().startswith('{') or 'query' in query_upper or 'mutation' in query_upper:
        return 'graphql'
    elif any(method in query_upper for method in ['GET', 'POST', 'PUT', 'PATCH', 'DELETE']) or query.startswith('http'):
        return 'rest'
    else:
        return 'generic'
...
def _highlight_query(query: str, query_type: str) -> str:
    ...
    if query_type == 'sql':
        sql_keywords = ['SELECT', 'FROM', 'WHERE', 'JOIN', 'LEFT JOIN', 'RIGHT JOIN', 'INNER JOIN', 
                       'GROUP BY', 'ORDER BY', 'HAVING', 'LIMIT', 'AND', 'OR', 'IN', 'NOT', 'NULL']
        for keyword in sql_keywords:
            highlighted_query = highlighted_query.replace(
                keyword, 
                f"{Colors.BOLD}{Colors.BRIGHT_BLUE}{keyword}{Colors.RESET}"
            )
        ...
        for keyword in graphql_keywords:
            highlighted_query = highlighted_query.replace(
                keyword, 
                f"{Colors.BOLD}{Colors.BRIGHT_CYAN}{keyword}{Colors.RESET}"
            )
        ...
        for keyword in rest_keywords:
            highlighted_query = highlighted_query.replace(
                keyword, 
                f"{Colors.BOLD}{Colors.BRIGHT_GREEN}{keyword}{Colors.RESET}"
            )
