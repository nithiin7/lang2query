...
module level: each tool's parameter list is the LLM-facing call contract (it
becomes the tool's JSON schema via bind_tools), so a retriever instance can't
be added as a normal parameter without asking the LLM to supply it. Binding
...
def make_retriever_tools(retriever) -> Dict[str, BaseTool]:
    ...
    Returns:
        Dict mapping tool name -> BaseTool, so callers can pick the subset
        they need (e.g. MetadataAgent uses the search/enumeration tools,
        HumanInTheLoopAgent uses only the validation tools).
    """
    ...
    def get_columns_by_table(database_name: str, table_names: List[str]) -> str:
        ...
        Returns:
            ...
            - results: Dictionary mapping table_name -> array of column objects with:
                * column_name, data_type, key_type (PRI/UNI/MUL), nullable, description, category
                (default_value and extra are not captured by the schema docs and are always null)
    ...
    return {
        ...
        "search_by_database": search_by_database,
        "search_by_table": search_by_table,
        "get_all_databases": get_all_databases,
        "count_databases": count_databases,
        "get_tables_in_database": get_tables_in_database,
        "count_tables_in_database": count_tables_in_database,
        "search_tables_in_databases": search_tables_in_databases,
        "complex_filter_search": complex_filter_search,
        "get_columns_by_table": get_columns_by_table,
        "validate_database_exists": validate_database_exists,
        "validate_table_exists": validate_table_exists,
        "search_similar_tables": search_similar_tables,
    }


__all__ = ["make_retriever_tools"]
