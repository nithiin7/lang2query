...
├── retreiver/        # Ingestion (chunking + embedding) pipeline. [Known misspelling — see Section 6.]
...
These are verified against the actual code, not guesses — treat this as the starting punch list.

1. **Broken import path: `retreiver` vs `retriever`.** The ingestion/retrieval directory is spelled `src/retreiver/` on disk, but nearly every import in the codebase references `retriever` (correct spelling) — `workflow.py`, all of `tools/retriever_tools.py`, `agents/schema_builder.py`. These imports currently fail at runtime. Needs either a directory rename (`retreiver` → `retriever`, then fix any path strings/configs that reference the old name) or consistent import fixes — a rename is the cleaner fix since the misspelling is the actual defect.
2. **`src/retreiver/retrieve_sql_kb.py` does not define `SQLKnowledgeBaseRetriever`.** It's currently a near-duplicate of `create_sql_kb_embeddings.py` (defines `SQLKnowledgeBaseEmbedder` instead). Every caller expecting `SQLKnowledgeBaseRetriever` (with methods `semantic_search`, `search_by_database`, `search_by_table`, `get_all_databases`, `count_databases`, `get_tables_in_database`, `search_tables_in_databases`, `complex_filter_search`, `get_columns_by_table`) will fail. This class needs to be written from scratch, matching the method signatures already expected by `tools/retriever_tools.py`.
3. **Second naming mismatch**: `tools/__init__.py` and `agents/human_in_the_loop.py` import from `tools.retriever_tool` (singular), but the actual file is `tools/retriever_tools.py` (plural).
4. **`retriever_tools.py` recreates `SQLKnowledgeBaseRetriever` inside every tool function call** instead of reusing a shared instance (contrast with `workflow.py`'s correct shared-instance pattern in `__init__`). Once bugs #1–3 are fixed, this should be refactored to inject a shared retriever.
5. **No SQL safety guardrail.** `query_validator.py` checks semantic correctness against user intent via LLM-as-judge, but nothing enforces that generated SQL is read-only (no deny-list or allow-list on statement type). Needs a deterministic check (e.g. via `sqlglot`/`sqlparse`) rejecting anything that isn't a `SELECT`, applied after generation and before any execution, plus read-only DB credentials at the infra level if/when this connects to a real database.
6. **Blocking sync call inside an async WebSocket handler.** `src/api/routes/query.py`'s `_process_workflow_stream` calls `next(stream)` synchronously inside an `async def`, which blocks FastAPI's event loop for the duration of each agent's LLM call. Fine for single-user local use; will serialize/stall concurrent users in any multi-user deployment. Fix: run the generator via `asyncio.to_thread` / `run_in_executor`.
7. **No re-ranking or hybrid search** in the retrieval layer — pure dense vector similarity via ChromaDB. Acceptable for now given the small, well-structured knowledge base, but worth revisiting if the schema documentation grows significantly.
...
make embeddings                                # (re)build the vector KB from src/retreiver/input/*.md
