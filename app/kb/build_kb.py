...
from app.config import KB_CONFIG, TABLE_CONFIG, get_database_config, get_model_config
...
def read_sql(table):
    ...
    query = f"SELECT * FROM {table} ORDER BY RAND() LIMIT 5;"
...
def get_model_for_kb():
    ...
    if kb_model_config["provider"] == "anthropic":
        return ChatAnthropic(
            temperature=kb_model_config["temperature"], model_name=kb_model_config["model"]
        )
        ...
        return ChatAnthropic(temperature=0.4, model_name="claude-3-5-sonnet-20240620")
...
chain = (
    RunnableMap(
        {"description": lambda x: x["description"], "data_sample": lambda x: x["data_sample"]}
    )
    | template
    | model
    | StrOutputParser()
...
def build_knowledge_base():
    ...
    for table_name, description in tqdm.tqdm(table_description.items()):
        try:
            ...
            response = chain.invoke({"description": description, "data_sample": data_dict}).replace(
                "```", ""
            )
